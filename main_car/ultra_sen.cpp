...
{
  pinMode(ULTRA_SEN_ECHO, INPUT);                      // Define the ultrasonic echo input pin
  pinMode(ULTRA_SEN_TRIG, OUTPUT);                     // Define the ultrasonic trigger input pin
...
float ultra_sen_dist_check(int serv_angle)
{
  ...
  //Serial.println(serv_angle);
  myservo.write(serv_angle);
  delay( abs(prev_angle - serv_angle) + 15);        // Wait serv reach that angle
