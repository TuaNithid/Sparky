...
INFRD_STATUS infrd_check_obstacle(void)
{
  ...
  if (infrd_r_sensor == HIGH && infrd_l_sensor == HIGH)       //Obstacle in right and left
  {
    infrd_ret = INFRD_BOTH;
    Serial.println("[INFRD] BOTH DETECTED!!");
  }                             
  else if (infrd_l_sensor == LOW && infrd_r_sensor == HIGH)   //Obstacle in right
  {
    infrd_ret = INFRD_RIGHT;
    Serial.println("[INFRD] RIGHT DETECTED!!");    
  }
  else if (infrd_r_sensor == LOW && infrd_l_sensor == HIGH)   //Obstacle in left	
  {
    infrd_ret = INFRD_LEFT;
    Serial.println("[INFRD] LEFT DETECTED!!");
  }
  else
